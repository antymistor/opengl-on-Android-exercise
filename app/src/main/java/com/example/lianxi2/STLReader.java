...
public class STLReader {
    ...
    private void parseModel(Model model, byte[] facetBytes) {
        ...
        float[] colors=new float[facetCount*3*4];
        int stlOffset = 0;
        try {
            for (int i = 0; i < facetCount; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    if (j == 0) {//法向量
                        ...
                        colors[i * 12 + (j - 1) * 4] =x/model.maxX;
                        colors[i * 12 + (j - 1) * 4 + 1] = y/model.maxY;
                        colors[i * 12 + (j - 1) * 4 + 2] =z/model.maxZ;
                        colors[i * 12 + (j - 1) * 4 + 3] =1.0f;
                    }
                }
